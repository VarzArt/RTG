...
const centerCremlin = document.querySelector('.cremlin__image-center').childNodes;
...
function setRandomColors (isInitial) {
	...
	cols.forEach((col, index) => {
		...
		for(let i = 0; i < leftCremlin.length; i++) {
			if(leftCremlin[i].nodeName.toLowerCase() == 'p' 
		...
		for(let j = 0; j < centerCremlin.length; j ++) {
			if(centerCremlin[j].nodeName.toLowerCase() == 'p'
